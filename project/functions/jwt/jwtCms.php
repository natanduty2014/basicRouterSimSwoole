...
class jwtCms
{
    ...
    private static function generatorprivate($data, $user_permissions = null)
    {
        ...
        $payload = [
            ...
            "user_id" => $data['use_id'],
            "user_con_id" => $data['use_con_id'],
            "user_name" => $data['use_name'],
            "user_email" => $data['use_email'],
            "user_usg_id" => $data['use_usg_id'],
            "user_avatar" => $data['use_avatar'],
            "user_role" => $data['usg_title'] ?? null,
            // Multi-unidade (escopo). Admin do contratante = is_admin=true (acessa
            // todas as unidades do contratante). Demais usuários têm unidades
            // explícitas via tb_users_rel_unidades.
            "user_is_admin" => !empty($data['is_admin']),
            "user_unidades" => isset($data['unidades']) && is_array($data['unidades'])
                ? array_values($data['unidades'])
                : [],
            'exp' => $time,
