...
class TinyStdoutLogger implements StdoutLoggerInterface
{
    ...
    public function log($level, $message, array $context = []): void
    {
        ...
        $line = sprintf('[%s][%s] %s %s', date('c'), $level, $msg, $this->contextToString($context));
        error_log($line);
    }
    ...
    private function contextToString(array $context): string
    {
        ...
        // Converte contexto em string simples para não quebrar
        $safe = [];
        foreach ($context as $k => $v) {
            if (is_scalar($v) || is_null($v)) {
                $safe[$k] = $v;
            } else {
                $safe[$k] = json_encode($v);
            }
        }
